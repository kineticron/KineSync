...
export function initLyricsLayout(
  ...
  rows = source.map((line, index) => {
    ...
    const group = lines.filter((runtime) => runtime.sourceIndex === index && !runtime.DotLine);
...
export function scrollToActiveLine(
  ...
  const state = getPlaybackWindowState(position, lyrics, backgrounds, timing);
  ...
  if (!range) return;
  const topOffset = viewport.closest(".landscape") ? LANDSCAPE_ACTIVE_LINE_TOP_OFFSET : 0;
  ...
  if (layoutDirty) {
    ...
    const creditsLayout = hasCredits && credits
  ...
  const credits = document.getElementById("spicyCredits");
  if (credits && !credits.hidden && position >= lyrics[lyrics.length - 1].lineEndTime) {
    target = getCreditsAwareScrollOffset({
      range, lyricsLength: lyrics.length, listHeight: height,
      creditsLayout: { top: credits.offsetTop, bottom: credits.offsetTop + credits.offsetHeight },
